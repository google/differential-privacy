...
var (
	example = flag.String("example", "count", "Example ID:\n"+
	...
	inputFile       = flag.String("input_file", "main/day_data.csv", "Input csv file name with raw data.")
	outputStatsFile = flag.String("output_stats_file", "stats.csv", "Output csv file name for stats results.")
	outputChartFile = flag.String("output_chart_file", "chart.png", "Output png file name for chart with stats.")
)
...
func main() {
	...
	outputStatsFileNewDP := strings.ReplaceAll(*outputStatsFile, path.Ext(*outputStatsFile), "_new"+path.Ext(*outputStatsFile))
	outputChartFileNewDP := strings.ReplaceAll(*outputChartFile, path.Ext(*outputChartFile), "_new"+path.Ext(*outputChartFile))
	...
	newDpOutput := runNewDPExample(s, visits, *example)
	...
	writeOutput(s, newDpOutput, outputStatsFileNewDP)
	...
	newDpHourToValue, err := readOutput(outputStatsFileNewDP)
	if err != nil {
		log.Exitf("Reading output text file (%s) to plot bar charts failed: %v", outputStatsFileNewDP, err)
	}
	...
	if err = drawPlotThreeWay(hourToValue, dpHourToValue, newDpHourToValue, *example, outputChartFileNewDP); err != nil {
		log.Exitf("Drawing bar chart failed: %v", err)
	}
...
func runNewDPExample(s beam.Scope, col beam.PCollection, example string) beam.PCollection {
	switch example {
	case count:
		return codelab.PrivateCountNewVisitsPerHour(s, col)
	default:
		log.Exitf("Unknown example %q specified, please use one of 'count'", example)
		return beam.PCollection{}
	}
}
