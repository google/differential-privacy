...
// greater than or equal to 1 - alpha using the confidence intervals for sum and count.
//
...
// computeConfidenceIntervalSplit computes a confidence interval for mean with a given 1 - countAlpha confidence level for count.
// It computes the corresponding alpha for sum to return a confidence interval with a confidence level of at least 1 - meanAlpha.
//
// Note Result() needs to be called before computeConfidenceIntervalSplit, otherwise this will return an error.
func (bm *BoundedMeanFloat64) computeConfidenceIntervalSplit(meanAlpha, countAlpha float64) (noise.ConfidenceInterval, error) {
	if !bm.resultReturned {
		return noise.ConfidenceInterval{}, fmt.Errorf("You need to call Result() before calling computeConfidenceIntervalSplit()")
	}
