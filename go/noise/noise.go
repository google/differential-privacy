...
type Noise interface {
	...
	// computeConfidenceIntervalInt64 computes a confidence interval that contains the true value X from which int64 noisedX
	// was computed with probablility greater or equal to the given confidenceLevel with the given noise parameters.
	computeConfidenceIntervalInt64(noisedX, l0Sensitivity, lInfSensitivity int64, epsilon, delta, confidenceLevel float64) (ConfidenceInterval, error)
	...
	computeConfidenceIntervalFloat64(noisedX float64, l0Sensitivity int64, lInfSensitivity, epsilon, delta, confidenceLevel float64) (ConfidenceInterval, error)
}
