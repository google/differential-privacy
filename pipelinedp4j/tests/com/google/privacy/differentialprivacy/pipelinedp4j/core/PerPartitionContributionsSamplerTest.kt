...
class PerPartitionContributionsSamplerTest {
  @Test
  fun sampleContributions_returnsSubsampleOfContributionsPerPartitionSinglePrivacyId() {
    val contributions = listOf(1.0, 2.0, 3.0, 4.0)
    val inputData =
      LocalCollection(
        sequenceOf(
    ...
    val returnedContributions = sampledData.data.toMap().getValue("pk1").valuesList

    assertThat(returnedContributions.count()).isEqualTo(2)
    // Returned partition keys are all in the list of the contributed partition keys.
    assertThat(contributions).containsAtLeastElementsIn(returnedContributions)
  }
  ...
  fun sampleContributions_returnsOriginalContributionsPerPartition() {
    ...
    assertThat(sampledData.data.toList())
      .containsExactly(
        Pair("pk1", privacyIdContributions { values += listOf(1.0, 2.0, 3.0) }),
        Pair("pk2", privacyIdContributions { values += listOf(4.0, 5.0, 6.0) }),
        Pair("pk1", privacyIdContributions { values += listOf(7.0) }),
      )
  }
  ...
  fun sampleContributions_returnsSubsampleOfContributionsPerPartition() {
    ...
    for (pk in returnedPks) {
      assertThat(sampledData.data.toMap().getValue(pk).valuesList).hasSize(1)
    }
  }
  ...
  fun sampleContributions_samplesFromManyContributions() {
    ...
    val sampledData =
      PerPartitionContributionsSampler(
        ...
        .sampleContributions(inputData) as LocalTable<String, PrivacyIdContributions>
    val returnedContributions = sampledData.data.toMap().getValue("pk").valuesList
