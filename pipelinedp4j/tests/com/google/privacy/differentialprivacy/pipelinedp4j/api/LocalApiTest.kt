...
class LocalApiTest {
  ...
  fun build_groupSelectionOnlyAndGroupsArePublic_throwsException() {
    ...
    val queryBuilder =
      LocalQueryBuilder.from(
        ...
        .groupBy({ it.groupKey }, GroupsType.PublicGroups.create(sequenceOf()))
  ...
  fun run_withFullTestMode_addsNoNoiseAndDoesNotPerformContributionBounding() {
    ...
    val result: Sequence<QueryPerGroupResult<String>> = query.run(testMode = TestMode.FULL)
  ...
  private fun createPublicGroups(publicGroupKeys: List<String>) = publicGroupKeys.asSequence()

  private fun assertEquals(
    result: Sequence<QueryPerGroupResult<String>>,
    expected: List<QueryPerGroupResultWithTolerance>,
  ) {
    val resultAsList = result.toList()
