          ...
          when (metric.type) {
            ...
            COUNT -> {
              ...
              throw IllegalArgumentException(
                "metric ${metric.type} is not allowed in non-feature metrics."
              )
          }
        }
        .toMutableList()
          ...
          when (metric.type) {
            ...
            MEAN -> {
              if (!varianceInMetrics) {
                val (countBudget, sumBudget) =
                  calculateCountSumBudgetsForMean(params, metric, feature as ScalarFeatureSpec)
                MeanCombiner(
                  params,
                  countBudget,
                  sumBudget,
  ...
  private fun calculateCountSumBudgetsForMean(
    ...
    val countBudgetSpec: BudgetPerOpSpec =
      if (meanDefinition.budgetSpec != null) {
        ...
        // Or COUNT spec from the same feature, or a global COUNT spec, or the default budget spec.
        getMetricDefinitionFromNonFeature(COUNT)?.budgetSpec
          ?: RelativeBudgetPerOpSpec(weight = 1.0)
      }

    // Budget spec for SUM.
    // SUM can only be a feature metric, so we only look in the current feature.
    val sumBudgetSpec: BudgetPerOpSpec =
      if (meanDefinition.budgetSpec != null) {
        // It is 50% of MEAN spec, if MEAN spec is provided.
        meanDefinition.budgetSpec!!.times(0.5)
      } else {
        // Or SUM spec from the same feature, or the default budget spec.
