...
class SparkTable<K, V>(val data: Dataset<Pair<K, V>>,
    ...
    private fun filterKeysStoredInSparkCollection(
        ...
        val tupleDataset = data.map(MapFunction {kv: Pair<K, V> -> kv.toTuple2()}, Encoders.tuple(keyEncoder, valueEncoder))
        val allowedKeysTupleDataset = allowedKeysDataset.map(MapFunction {kv: Pair<K, String> -> kv.toTuple2()}, Encoders.tuple(keyEncoder, Encoders.STRING()))
        val filteredTupleDataset = tupleDataset.joinWith(allowedKeysTupleDataset, tupleDataset.col("_1").equalTo(allowedKeysTupleDataset.col("_1")),
            "inner" )
        val filteredPairDataset= filteredTupleDataset.map(MapFunction { kv: Tuple2<Tuple2<K, V>, Tuple2<K, String>> -> Pair(kv._1._1, kv._1._2)}, keyValueEncoder)
