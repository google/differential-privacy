...
def convolve_boundary_masses(
  ...
  """Computes boundary masses (p_min, p_max) for the convolution Z = X + Y.
  ...
  Args:
    p_min_1: Lower boundary mass of first distribution.
    p_max_1: Upper boundary mass of first distribution.
    p_min_2: Lower boundary mass of second distribution.
    p_max_2: Upper boundary mass of second distribution.
    domain: Domain of distributions (REALS or POSITIVES).

  Returns:
    Tuple of (p_min, p_max) convolved boundary masses.
  """
...
def binary_self_convolve(
  ...
  """Self-convolves via exponentiation by squaring, using a convolve callback.

  Algorithm 3 (`self-conv`) in Appendix C of https://arxiv.org/abs/2602.17284.

  Args:
    dist: Distribution to self-convolve.
    ...
    convolve: Two-distribution convolution callable.
...
def exp_linear_to_geometric(
  ...
  """Maps a linear-grid distribution to a geometric-grid one via exp(.).

  Maps REALS domain → POSITIVES domain.
  The −∞ atom (p_min in REALS) maps to the 0 atom (p_min in POSITIVES).

  Args:
    dist: Input linear-grid distribution.

  Returns:
    The transformed geometric-grid DenseDiscreteDist.
  """
...
def log_geometric_to_linear(
  ...
  """Maps a geometric-grid distribution to a linear-grid one via log(.).

  Maps POSITIVES domain → REALS domain.
  The 0 atom (p_min in POSITIVES) maps to the −∞ atom (p_min in REALS).

  Args:
    dist: Input geometric-grid distribution.

  Returns:
    The transformed linear-grid DenseDiscreteDist.
  """
...
def calc_pld_dual(
  ...
  """Computes the paper PLD dual ``D(L)`` (Definition 3.1).
  ...
  Args:
    realization: Input PLDRealization object.

  Returns:
    The dual PLDRealization.
  """
