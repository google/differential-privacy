"""Random-allocation composition internals (realizations, Gaussian path)."""
...
def _linear_dist_to_dp_accounting_pmf(
  ...
  Args:
      dist: Linear-grid loss distribution compatible with dp_accounting.
          ...
          ``lower_loss`` index.
      pessimistic_estimate: Whether to use pessimistic estimate in
          dp_accounting.
...
def realization_remove_base_distributions(
...
) -> tuple[distributions.DenseDiscreteDist, distributions.DenseDiscreteDist]:
  """Prepare remove-direction factors from a loss-space realization.

  Algorithm 1 (`rand-alloc-rem`) in Appendix C of
  ...
  # come first for DOMINATES, but dual derivation must come first for
  # IS_DOMINATED.
  if bound_type == definitions.BoundType.DOMINATES:
    ...
    coarsened_base = distributions.rediscretize_dist(
        dist=realization,
    ...
    utils.validate_dense_linear_dist(coarsened_base)
    base_realization = distributions.PLDRealization.from_linear_dist(
        coarsened_base
    )
    neg_dual_dist = utils.negate_reverse_linear_distribution(
        utils.calc_pld_dual(base_realization)
    )
  ...
  dual_realization = utils.calc_pld_dual(realization)
  neg_dual_linear = utils.negate_reverse_linear_distribution(dual_realization)
  # Avoid inflating the grid when the target is finer than the original one.
  effective_disc = max(realization.step, loss_discretization)
  lower_realization_input = distributions.DenseDiscreteDist(
  ...
  lower_base_dist = distributions.rediscretize_dist(
  ...
  utils.validate_dense_linear_dist(lower_base_dist)
  neg_dual_dist = distributions.rediscretize_dist(
  ...
  utils.validate_dense_linear_dist(neg_dual_dist)
...
def realization_add_base_distribution(
  ...
  Algorithm 2 (`rand-alloc-add`) in Appendix C of
  https://arxiv.org/abs/2602.17284.
  ...
  # Avoid inflating the grid when the target is finer than the original one.
  effective_disc = max(realization.step, loss_discretization)
  coarsened = distributions.rediscretize_dist(
      dist=realization,
      tail_truncation=tail_truncation,
      loss_discretization=effective_disc,
      spacing_type=definitions.SpacingType.LINEAR,
  ...
  utils.validate_dense_linear_dist(coarsened)
  return coarsened
...
def allocation_full_pld(
    *,
    compute_base_pld_remove: Callable[..., distributions.DenseDiscreteDist],
    compute_base_pld_add: Callable[..., distributions.DenseDiscreteDist],
    num_steps: int,
    num_selected: int,
    num_epochs: int,
    loss_discretization: float,
  ...
  """Orchestrates full allocation PLD construction for both directions.
  ...
  Args:
    compute_base_pld_remove: Callable computing base PLD for remove direction.
    compute_base_pld_add: Callable computing base PLD for add direction.
    num_steps: Total number of steps.
    num_selected: Number of steps selected per epoch.
    num_epochs: Number of epochs.
    loss_discretization: Discretization step size for loss values.
    tail_truncation: Truncation threshold for probability mass in tails.
    bound_type: Bound type (DOMINATES or IS_DOMINATED).

  Returns:
    The composed PrivacyLossDistribution object.
  """
  # Input validation
  utils.validate_allocation_params(num_steps, num_selected, num_epochs)
  utils.validate_discretization_params(loss_discretization, tail_truncation)
...
def _allocation_directional_pld(
    *,
    compute_base_pld: Callable[..., distributions.DenseDiscreteDist],
    num_steps: int,
    num_selected: int,
    num_epochs: int,
    loss_discretization: float,
  ...
  """Builds one-direction allocation PLD with adaptive floor/ceil decomposition.
  ...
  Args:
    compute_base_pld: Callable computing base directional PLD.
    num_steps: Total number of steps.
    num_selected: Number of steps selected per epoch.
    num_epochs: Number of epochs.
    loss_discretization: Discretization step size for loss values.
  ...
  Returns:
    The directional DenseDiscreteDist.

  Raises:
    ValueError: If parameters are invalid or floor/ceil steps mismatch.
    RuntimeError: If failed to build either floor or ceil component.
  """
  # Input validation
  utils.validate_allocation_params(num_steps, num_selected, num_epochs)
  utils.validate_discretization_params(loss_discretization, tail_truncation)
  ...
  # Tail: active tail-consuming ops = one _allocation_directional_pld_core
  # per component plus one fft_convolve when both components are active,
  # giving 2*component_count - 1 ops total (1 for component_count=1, 3 for
  # component_count=2).  After tail_truncation /= (2*component_count - 1),
  # each op consumes at most the rescaled budget, and all ops together sum
  # to <= (2*component_count - 1) * rescaled = tail_truncation.
  ...
  if dist_floor is None:
    if dist_ceil is None:
      raise RuntimeError(
          "allocation_directional_pld failed to build either floor or ceil "
          "component"
      )
    return dist_ceil
  if dist_ceil is None:
  ...
  return convolution.fft_convolve(
      dist_1=dist_floor,
      dist_2=dist_ceil,
      tail_truncation=tail_truncation,
      bound_type=bound_type,
...
def geometric_allocation_pld_base_remove(
  ...
  """Builds the REMOVE component PLD via exp-space geometric composition.

  The callback ``base_distributions_creation`` provides one-step
  ``(base, neg_dual_base)`` factors, which are shifted and composed.

  Args:
    base_distributions_creation: Callable creating the base distribution pair.
  ...
  Returns:
    The computed REMOVE component DenseDiscreteDist.
  """
  # Input validation
  if num_steps < 1:
  ...
  utils.validate_discretization_params(loss_discretization, tail_truncation)
  utils.validate_bound_type(bound_type)
  # For num_steps > 1 there are active convolution stages beyond base
  # construction.  For num_steps == 1 neither convolution stages nor Phases
  # 2/3 execute, so no tail-budget division is needed.
  if num_steps > 1:
    # Tail: three phases each receive tail_truncation / 3 after division.
    # Contributions to output p_max (letting T = num_steps, budget/3 =
    # rescaled per-phase share):
    #   Phase 1 (base_distributions_creation):
    #       base_factor_tail_truncation = budget/(3T).  neg_dual is
    #       self-convolved T-1 times, base convolved once; total amplified
    #       contribution = ((T-1) + 1) * budget/(3T) = budget/3.
    #   Phase 2 (geometric_self_convolve, T-1): called with budget/3 ->
    #       <= budget/3.
    #   Phase 3 (geometric_convolve): called with budget/3 -> <= budget/3.
    # Sum <= budget/3 + budget/3 + budget/3 = budget
    tail_truncation /= 3
  # Each base factor is one of num_steps terms in the final product; its
  # individual tail error is amplified by num_steps through self-convolution,
  # so scale its budget down by num_steps so the amplified contribution stays
  ...
  # For num_steps == 1 the centering shift is log(1) = 0 and the exp/log
  # round-trip is an identity, so base is already the final result.
  if num_steps == 1:
    return base

  # Subtract the average loss
  log_num_steps = float(np.log(num_steps))
  centered_neg_dual = distributions.DenseDiscreteDist(
  ...
  centered_base = distributions.DenseDiscreteDist(
  ...
  exp_neg_dual = utils.exp_linear_to_geometric(centered_neg_dual)
  exp_base = utils.exp_linear_to_geometric(centered_base)

  # V_{t-1} <- self-conv(V1, t-1, ...).
  exp_convolved_dual = convolution.geometric_self_convolve(
      dist=exp_neg_dual,
      T=num_steps - 1,
      tail_truncation=tail_truncation,
      bound_type=bound_type,
  ...
  # U_t <- conv(V_{t-1}, U1, ...).
  exp_convolved = convolution.geometric_convolve(
  ...
  # L_t <- log(U_t).
  return utils.log_geometric_to_linear(exp_convolved)


def geometric_allocation_pld_base_add(
    *,
    base_distributions_creation: Callable[..., distributions.DenseDiscreteDist],
    num_steps: int,
    loss_discretization: float,
    tail_truncation: float,
    bound_type: definitions.BoundType,
) -> distributions.DenseDiscreteDist:
  """Builds the ADD component PLD via exp-space geometric self-composition.
  ...
  Args:
    base_distributions_creation: Callable creating the one-step ADD factor.
    num_steps: Number of steps.
    loss_discretization: Discretization step size for loss values.
    tail_truncation: Truncation threshold for probability mass in tails.
    bound_type: Bound type (DOMINATES or IS_DOMINATED).
  ...
  Returns:
    The computed ADD component DenseDiscreteDist.
  """
  # Input validation
  utils.validate_discretization_params(loss_discretization, tail_truncation)
  utils.validate_bound_type(bound_type)
  ...
  # For num_steps > 1 there are active convolution stages beyond base
  # construction.  For num_steps == 1 neither convolution stages nor Phase 2
  # execute, so no tail-budget division is needed.
  if num_steps > 1:
    # Tail: after dividing by 2, each of the two phases receives
    # tail_truncation:
    #   Phase 1 (base creation): tail_truncation / num_steps per call;
    #       num_steps-fold self-convolution amplifies the contribution back
    #       to tail_truncation.
    ...
    tail_truncation /= 2
  # Each base factor's tail error is amplified by num_steps through
  # self-convolution, so scale its budget down by num_steps so the amplified
  # contribution stays within the phase budget.
  base_factor_tail_truncation = tail_truncation / num_steps
  ...
  neg_base = utils.negate_reverse_linear_distribution(base)
  centered_neg_base = distributions.DenseDiscreteDist(
  ...
  # Factor preparation in exp-space.
  exp_base = utils.exp_linear_to_geometric(centered_neg_base)
  exp_bound_type = (
      definitions.BoundType.IS_DOMINATED
      if bound_type == definitions.BoundType.DOMINATES
      else definitions.BoundType.DOMINATES
  )
  # U_t <- self-conv(U, t, lower).
  exp_convolved = convolution.geometric_self_convolve(
      dist=exp_base,
      T=num_steps,
      tail_truncation=tail_truncation,
  ...
  # L_t <- -log(U_t).
  log_dist = utils.log_geometric_to_linear(exp_convolved)
  return utils.negate_reverse_linear_distribution(log_dist)
...
def gaussian_allocation_pld_core(
  ...
  """Routes one Gaussian component through the GEOM backend.
  ...
  Returns:
    The computed DenseDiscreteDist.
  """
  # Input validation
  if num_steps < 1:
    raise ValueError(f"num_steps must be >= 1, got {num_steps}")
  ...
  utils.validate_discretization_params(loss_discretization, tail_truncation)
  utils.validate_bound_type(bound_type)
  if direction not in (definitions.Direction.ADD, definitions.Direction.REMOVE):
...
def _allocation_directional_pld_core(
  ...
  """Builds and finalizes one floor/ceil decomposition component.
  ...
  Args:
    compute_base_pld: Callable computing the base PLD component.
    num_steps: Number of steps.
    num_epochs: Number of epochs.
    loss_discretization: Discretization step size for loss values.
    tail_truncation: Truncation threshold for probability mass in tails.
  ...
  Returns:
    The finalized linear DenseDiscreteDist.
  """
  # Tail: divide by 3; each of the three phases receives tail_truncation
  # (the divided value):
  #   Phase 1 (base creation, amplified by num_epochs): base construction
  #       and truncate_edges each use base_tail_truncation.  Amplified
  #       total: num_epochs * 2 * tail_truncation / (2 * num_epochs) =
  #       tail_truncation.
  ...
  # Sum: tail_truncation + tail_truncation + tail_truncation =
  #     3 * tail_truncation = original ✓
  tail_truncation /= 3
  ...
  utils.validate_dense_linear_dist(prepared_base_dist)

  if num_epochs == 1:
    composed_dist = prepared_base_dist
  else:
    composed_dist = convolution.fft_self_convolve(
  ...
  utils.validate_dense_linear_dist(final_dist)
  return final_dist


def _compose_full_pld(
    *,
    remove_dist: distributions.DenseDiscreteDist | None,
    add_dist: distributions.DenseDiscreteDist | None,
    bound_type: definitions.BoundType,
) -> privacy_loss_distribution.PrivacyLossDistribution:
  ...
  pessimistic_estimate = bound_type == definitions.BoundType.DOMINATES
...
def _gaussian_remove_geom_loss_factors(
  ...
  dual_grid = distributions.discretize_aligned_grid(
      x_min=dual_x_min,
      x_max=dual_x_max,
  ...
  base_grid = distributions.discretize_aligned_grid(
      x_min=base_x_min,
      x_max=base_x_max,
      spacing_type=definitions.SpacingType.GEOMETRIC,
      align_to_multiples=True,
  ...
  dual_factor_dist = distributions.discretize_on_grid(
      dist=exp_dual,
      grid=dual_grid,
      bound_type=bound_type,
      pmf_min_increment=factor_tail_truncation,
  )
  utils.validate_dense_geometric_dist(dual_factor_dist)

  base_factor_dist = distributions.discretize_on_grid(
      dist=exp_base,
      grid=base_grid,
      bound_type=bound_type,
      pmf_min_increment=factor_tail_truncation,
  )
  utils.validate_dense_geometric_dist(base_factor_dist)

  dual_loss_factor = utils.log_geometric_to_linear(dual_factor_dist)
  base_loss_factor = utils.log_geometric_to_linear(base_factor_dist)
...
def _gaussian_add_geom_loss_factor(
  ...
  base_dist = distributions.discretize_on_grid(
      dist=base_lognorm,
      grid=base_grid,
      bound_type=bound_type,
      pmf_min_increment=tail_truncation,
  )
  utils.validate_dense_geometric_dist(base_dist)
  return utils.log_geometric_to_linear(base_dist)
