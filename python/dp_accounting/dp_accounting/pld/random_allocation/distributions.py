...
# =============================================================================
# Pre-class helpers (used by class constructors)
# =============================================================================


def _strip_zero_edges(prob_arr: np.typing.NDArray) -> tuple[int, int]:
  """Returns (min_ind, max_ind) of the nonzero range in prob_arr.

  Args:
    prob_arr: Array of probability values.

  Returns:
    Tuple of (min_ind, max_ind) indices.

  Raises:
    ValueError: If all mass is zero.
  """
...
def _zero_mass(
  ...
  """Removes mass from one side of ``values``, selected by ``from_left``.
  ...
  Args:
    ...
    mass: Mass to remove.
    from_left: Whether to remove mass from the left (True) or right (False).
    exact: Whether to partially consume the pivot bin to remove exact mass.

  Returns:
    Array with specified mass removed.
  """
  if mass <= 0.0:
    return values
...
def _exp_moment_terms(
  ...
  """Returns per-bin contributions to ``E[exp(-X)]``.
  ...
  Args:
    prob_arr: Array of probability values.
    x_vals: Array of grid values.

  Returns:
    Per-bin contributions array.

  Raises:
    ValueError: If prob_arr and x_vals shapes do not match.
  """
...
def _compute_truncation(
  ...
  """Computes truncated distribution parameters without creating objects.
  ...
  Args:
    ...
    p_min: Probability mass at minimum boundary.
    p_max: Probability mass at maximum boundary.
    tail_truncation: Probability mass to truncate in the tails.
    bound_type: Bound type (DOMINATES or IS_DOMINATED).

  Returns:
    Tuple of (new_prob_arr, new_p_min, new_p_max, min_ind, max_ind) where
    min_ind and max_ind are indices into the original prob_arr.
  """
...
class DiscreteDistBase(abc.ABC):
  ...
  def _validate_basic(self) -> None:
    """Validates PMF probabilities, boundary masses, and total mass."""
...
class DenseDiscreteDist(DiscreteDistBase):
  ...
  def _validate_grid(self) -> None:
    """Validates grid spacing, domain consistency, and parameters."""
...
def enforce_mass_conservation(
  ...
  """Enforces total mass with one bound-type-selected boundary held fixed.
  ...
  Args:
    prob_arr: Array of probability values.
    expected_p_min: Expected probability mass at minimum boundary.
    expected_p_max: Expected probability mass at maximum boundary.
    bound_type: Bound type (DOMINATES or IS_DOMINATED).

  Returns:
    Tuple of (adjusted_prob_arr, p_min, p_max) with total mass 1.
  """
...
def _adaptive_bins_from_cdf(
  ...
  Args:
    cdf: Evaluated CDF array.
    tail_truncation: Truncation threshold for accumulating bin mass.

  Returns:
    Array of bin probabilities.
  """
  n = cdf.size
...
def _adaptive_bins_from_sf(
  ...
  Args:
    sf: Evaluated survival function array.
    tail_truncation: Truncation threshold for accumulating bin mass.
...
def _stable_cdf_and_sf(
  ...
  """Computes numerically stable CDF and survival function on a grid.

  Args:
    dist: Continuous distribution or frozen scipy distribution.
    x_array: Grid points.

  Returns:
    Tuple of (cdf, sf) arrays evaluated on x_array.
  """
  median = dist.median()
...
def _compute_discrete_prob(
  ...
  """Computes bin probabilities via adaptive CDF/SF increments (logcdf/logsf).
  ...
  Args:
    ...
    x_array: Grid points.
    bound_type: Bound type (DOMINATES or IS_DOMINATED).
    pmf_min_increment: Minimum CDF/SF increment for bin mass.

  Returns:
    Tuple of (bin_probs, p_left, p_right).
  """
...
def _remap_prob_to_grid(
  ...
  Args:
    x_array: Input support points.
    prob_arr: Input PMF probabilities.
    x_array_out: Target grid support points.
    dominates: Whether remap must dominate (True) or be dominated (False).

  Returns:
    Remapped probability array on x_array_out.
  """
...
def rediscretize_dist(
  ...
  """Rediscretizes a distribution onto a requested grid spacing.
  ...
  Args:
    dist: Distribution to rediscretize.
    tail_truncation: Probability mass to truncate in the tails.
    loss_discretization: Discretization step size for the new grid.
    spacing_type: Grid spacing type (LINEAR or GEOMETRIC).
    bound_type: Bound type (DOMINATES or IS_DOMINATED).

  Returns:
    Rediscretized DenseDiscreteDist.
  """
