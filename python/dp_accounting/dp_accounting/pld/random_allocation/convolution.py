...
def _check_fft_memory(fft_size: int, label: str = "FFT") -> None:
  ...
  if estimated_bytes > MAX_FFT_BYTES:
    raise MemoryError(
        ...
        "Reduce grid size, increase loss_discretization, or raise "
        "MAX_FFT_BYTES."
    )
...
def fft_convolve(
  ...
  if not (
      isinstance(dist_1, distributions.DenseDiscreteDist)
      and dist_1.spacing_type == definitions.SpacingType.LINEAR
  ) or not (
      isinstance(dist_2, distributions.DenseDiscreteDist)
      and dist_2.spacing_type == definitions.SpacingType.LINEAR
  ):
    raise TypeError(
  ...
  if dist_1.domain != dist_2.domain:
    raise ValueError(
        f"Input domains must be identical, got {dist_1.domain} vs "
        f"{dist_2.domain}"
    )
  ...
  # Capture ghost-mass bounds and normalization factors before FFT buffers
  # are allocated.
...
def _calc_fft_window_size(
  ...
  """Calculate FFT window bounds for ``num_convolutions`` self-convolutions."""
  # ``compute_self_convolve_bounds`` gives a Chernoff-style window
  # [lower, upper] that should contain all but ``tail_truncation`` mass
  # after ``num_convolutions`` convolutions.
  ...
  if not 0 < window_size < float("inf"):
    ...
    warnings.warn(
        ...
        f"(tail_truncation={tail_truncation:.3e}, "
        f"num_convolutions={num_convolutions}). "
...
def _fft_self_convolve_direct(
  ...
  """Computes T-fold self-convolution directly in Fourier domain.

  Args:
    dist: The input dense discrete distribution.
    T: Number of self-convolutions.
    tail_truncation: Probability mass to truncate in the tails.
    bound_type: Bound type (DOMINATES or IS_DOMINATED).

  Returns:
    The self-convolved DenseDiscreteDist.
  """
  # Budget split: the input tail_truncation is divided into three equal
  # thirds.
  #   _calc_fft_window_size: Chernoff-based window determines the one-sided
  #       tail cutoff (right-tail for DOMINATES, folded-back mass bound for
  #       IS_DOMINATED).
  #   explicit opposite-side trim: left_tail_ind for DOMINATES (zeroes left
  #       bins, pushes mass to p_max) / right_tail_ind for IS_DOMINATED
  #       (zeroes right bins, pushes mass to p_min).
  #   final truncate_edges: trims actual near-zero edge bins the Chernoff
  #       window conservatively included on the remaining untrimmed side,
  #       reducing output bin count without sacrificing accuracy.
  ...
  # ``shift_left`` is the left edge of the retained convolution window.
  # Rolling aligns that window to index 0 so truncation logic can work
  # in-place.
...
def fft_self_convolve(
  ...
  if use_direct:
    try:
      ...
      warnings.warn(
          "fft_self_convolve: direct method exceeded "
          f"{MAX_FFT_BYTES / 1024**3:.0f} GB "
...
def _geometric_convolution_kernel(
  ...
  """Accumulates geometric-grid convolution terms with vectorized slices.
  ...
  Args:
    PMF_base: Base PMF array.
    PMF_scaled: Scaled PMF array.
    delta_lohi: Precomputed delta for low-high.
    delta_hilo: Precomputed delta for high-low.

  Returns:
    The accumulated output PMF array.
  """
  n = PMF_base.size
...
def _compute_geometric_convolution(
  ...
  """Aligns grids, computes bin mapping parameters, and invokes the kernel.

  Algorithm 4 (`conv`) with internal Algorithm 5 (`range-renorm`) in
  Appendix C of https://arxiv.org/abs/2602.17284.

  Args:
    x1: Support points of first distribution.
    p1: Probabilities of first distribution.
    x2: Support points of second distribution.
    p2: Probabilities of second distribution.
    geom_step: Geometric step ratio.
    bound_type: Bound type (DOMINATES or IS_DOMINATED).

  Returns:
    Tuple of (x_out, pmf_out) representing the convolved support and PMF.
  """
...
def _add_single_zero_atom_cross_term(
  ...
  """Adds mass from one zero atom and the other input's finite support.
  ...
  Args:
    pmf_conv: Convolved PMF array.
    x_arr: Support array of non-zero input.
    prob_arr: Probability array of non-zero input.
    zero_prob: Probability mass at zero for the other input.
    x_out_0: First point of output geometric grid.
    geom_step: Geometric step ratio.
    bound_type: Bound type (DOMINATES or IS_DOMINATED).

  Returns:
    Updated convolved PMF array including the cross term mass.
  """
  if zero_prob == 0.0:
    return pmf_conv
...
def geometric_convolve(
  ...
  """Convolves two geometric-grid distributions.
  ...
  Args:
    dist_1: First geometric distribution.
    dist_2: Second geometric distribution.
    tail_truncation: Probability mass to truncate in the tails.
    bound_type: Bound type (DOMINATES or IS_DOMINATED).

  Returns:
    The convolved DenseDiscreteDist.
  """
  # Input validation
  if not (
      isinstance(dist_1, distributions.DenseDiscreteDist)
      and dist_1.spacing_type == definitions.SpacingType.GEOMETRIC
      and dist_1.domain == distributions.Domain.POSITIVES
  ) or not (
      isinstance(dist_2, distributions.DenseDiscreteDist)
      and dist_2.spacing_type == definitions.SpacingType.GEOMETRIC
