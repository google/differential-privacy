...
class PldPrivacyAccountantTest(
  ...
  def test_exponential_mechanism_basic(self):
    ...
    event2 = dp_event.ExponentialMechanismDpEvent(2.0)
    accountant = pld_privacy_accountant.PLDAccountant()
    accountant.compose(event1)
    accountant.compose(event2)
    self.assertEqual(accountant.get_delta(3.0), 0.0)
  ...
  def test_permute_and_flip_basic(self):
    event1 = dp_event.PermuteAndFlipDpEvent(1.0)
    event2 = dp_event.PermuteAndFlipDpEvent(2.0)
    accountant = pld_privacy_accountant.PLDAccountant()
    accountant.compose(event1)
    accountant.compose(event2)
    # Must satisfy at least epsilon-DP with epsilon=3.
    self.assertLessEqual(accountant.get_delta(3.0), 0.0 + 1e-10)
    # Laplace PLD gives inf at delta=0 (unbounded support). At small delta,
    # epsilon should be close to but slightly below the pure-DP bound of 3.
    self.assertLess(accountant.get_epsilon(1e-10), 3.0 + 0.1)

  def test_permute_and_flip_tighter_than_exp_mech(self):
    # For fixed epsilon and delta > 0, permute-and-flip (Laplace PLD) should
    # give tighter epsilon than the exponential mechanism (black-box eps-DP).
    eps = 1.0
    target_delta = 1e-5
    pf_accountant = pld_privacy_accountant.PLDAccountant()
    pf_accountant.compose(dp_event.PermuteAndFlipDpEvent(eps))
    em_accountant = pld_privacy_accountant.PLDAccountant()
    em_accountant.compose(dp_event.ExponentialMechanismDpEvent(eps))
    self.assertLessEqual(
        pf_accountant.get_epsilon(target_delta),
        em_accountant.get_epsilon(target_delta),
    )
