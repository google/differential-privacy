...
from dp_accounting.pld.random_allocation import api as random_allocation_api
from dp_accounting.pld.random_allocation import definitions as random_allocation_definitions
      ...
      if not isinstance(event.event, dp_event.GaussianDpEvent):
        return CompositionErrorDetails(
            ...
            error_message=(
                ...
                f'`GaussianDpEvent`. Found {event.event}.'
            ),
        )
      if do_compose:
        if event.num_selected == 0:
          ...
          params = random_allocation_definitions.PrivacyParams(
              sigma=event.event.noise_multiplier,
              num_steps=event.num_steps,
              num_selected=event.num_selected,
          )
          config = random_allocation_definitions.AllocationSchemeConfig(
              value_discretization_interval=self._value_discretization_interval,
          )
          alloc_pld = random_allocation_api.gaussian_allocation_pld(
              params, config
          )
