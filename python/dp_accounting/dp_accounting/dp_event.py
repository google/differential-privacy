...
class DiscreteGaussianDpEvent(DpEvent):
  ...
  discrete Gaussian distribution with scale parameter σ. The (centered)
  discrete Gaussian distribution with scale parameter σ has probability
  mass function proportional to exp(-z@z / 2σ²) at z for any integer vector z.
  This is roughly equivalent to GaussianDpEvent with
