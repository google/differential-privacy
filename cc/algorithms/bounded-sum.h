...
class BoundedSumWithFixedBounds : public BoundedSum<T> {
 public:
  ...
  absl::StatusOr<ConfidenceInterval> GetOutputConfidenceInterval(
      const Output& output, double confidence_level) const override {
    return mechanism_->NoiseConfidenceInterval(confidence_level,
                                               GetValue<T>(output));
  }
...
class BoundedSumWithApproxBounds : public BoundedSum<T> {
 public:
  ...
  absl::StatusOr<ConfidenceInterval> GetOutputConfidenceInterval(
      const Output& output, double confidence_level) const override {
    if (!output.has_error_report() ||
        !output.error_report().has_bounding_report()) {
      return absl::InvalidArgumentError(
          "Expected Output proto to contain a bounding report, but it is "
          "missing. Check that the Output proto was generated by an Algorithm "
          "that uses approximate bounds rather than explicit bounds.");
    }
    BoundingReport report = output.error_report().bounding_report();
    T lower = GetValue<T>(report.lower_bound());
    T upper = GetValue<T>(report.upper_bound());
    ASSIGN_OR_RETURN(std::unique_ptr<NumericalMechanism> mechanism,
                     BuildNumericalMechanism(lower, upper));
    return mechanism->NoiseConfidenceInterval(confidence_level,
                                              GetValue<T>(output));
  }
  ...
  // Returns the epsilon used to calculate the noisy sum.  The overall
  // algorithm also uses epsilon for privately inferred bounds using approx
  // bounds.
 ...
 protected:
  absl::StatusOr<Output> GenerateResult(double noise_interval_level) override {
    ...
    // minimizes clamping and so maximizes accuracy. We need to be careful
    // with the numerical limits since -max == lowest + 1 for integers.
 ...
 private:
  // Build a numerical mechanism using the provided lower and upper bounds.
  absl::StatusOr<std::unique_ptr<NumericalMechanism>> BuildNumericalMechanism(
      T lower, T upper) const {
    return BoundedSum<T>::BuildMechanism(
